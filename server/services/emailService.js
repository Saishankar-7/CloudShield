...
/**
 * Initializes or retrieves the Nodemailer transporter instance.
 */
const getTransporter = async () => {
  ...
  if (smtpHost && emailUser && emailPass) {
    ...
    try {
      ...
      logger.error(`Nodemailer: Ethereal creation failed: ${etherealErr.message}`);
      transporter = {
        sendMail: async (mailOptions) => {
          logger.info(`[NODEMAILER SIMULATED] To: ${mailOptions.to} | Subject: ${mailOptions.subject}`);
...
const emailService = {
  ...
  sendDocumentAccessOtp: async ({ user, resource, otp, deviceInfo = {}, locationInfo = {} }) => {
    try {
      ...
      // If user is a demo account (@company.com/@example.com), deliver to configured EMAIL_USER so the tester receives it
      ...
      logger.error(`EmailService: Failed to send OTP email to ${user.email}: ${err.message}`);
      logger.info(`[FALLBACK LOG] Document MFA OTP for ${user.email}: ${otp}`);
  ...
  sendMfaSecurityOtp: async ({ user, otp, title = 'Security Verification', description = 'Account Authentication Challenge' }) => {
    try {
      ...
      logger.error(`EmailService: Failed to send MFA OTP email to ${user.email}: ${err.message}`);
      logger.info(`[FALLBACK LOG] MFA OTP for ${user.email}: ${otp}`);
      return { success: false, error: err.message, fallbackLogged: true };
    }
  },
};
